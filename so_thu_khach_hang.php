...
<style>
    *, *::before, *::after { 
    ...
    .filter-bar input[type="month"], .filter-bar input[type="text"] { 
        border: 1px solid #e5e7eb; 
        border-radius: 6px; 
        padding: 3px 8px; 
        height: 26px; 
...
<div class="page-wrapper">
    <div class="breadcrumb">
        ...
        <span>Số thu tiền khách hàng</span>
    </div>
    <div class="card">
        <div class="card-header">
            <h1>💰 Số thu tiền khách hàng</h1>
            <p>Ghi nhận và theo dõi các khoản thu từ khách hàng</p>
        </div>
        ...
        <div class="form-panel">
            <div class="form-col">
                <div class="col-title">📋 Thông tin khoản thu</div>
            ...
            <div class="form-col">
                ...
                <div class="field-row"><label>Số tiền phải thu:</label><input type="text" name="so_tien_phai_thu" id="so_tien_phai_thu" class="money-red" onkeyup="formatNum(this);tinhConLai();" placeholder="0"></div>
                <div class="field-row"><label>Số tiền đã thu:</label><input type="text" name="so_tien_da_thu" id="so_tien_da_thu" class="money-green" onkeyup="formatNum(this);tinhConLai();" placeholder="0"></div>
                <div class="field-row"><label>Còn lại (chưa thu):</label><input type="text" id="con_lai_display" class="con-lai-field" readonly></div>
        ...
        <div class="filter-bar">
            ...
            <label style="display:flex;align-items:center;gap:5px;"><input type="checkbox" id="f_chuathu" onchange="filterTable()"> Chỉ chưa thu đủ</label>
        ...
        <div class="toolbar">
            ...
            <div class="summary-group">
                <div class="sbox sbox-orange">Phải thu<span id="sum_phai">0</span></div>
                <div class="sbox sbox-green">Đã thu<span id="sum_da">0</span></div>
                <div class="sbox sbox-red">Còn lại<span id="sum_con">0</span></div>
            </div>
        </div>
        </form>
        ...
        <table id="mainTable">
        <thead><tr>
            <th width="40">Stt</th><th width="40">Chọn</th><th width="90">Ngày thu</th>
            <th width="150">Khách hàng</th><th width="110">Mã CT</th>
            <th width="180">Diễn giải</th><th width="130">Người trả</th>
            <th width="120">Phải thu</th><th width="120">Đã thu</th>
            <th width="100">Trạng thái</th><th>Ghi chú</th>
        </tr></thead>
        <tbody id="tableBody">
        <?php $stt=1; while($r=$res->fetch_assoc()):
            $con_lai = $r['so_tien_phai_thu'] - $r['so_tien_da_thu'];
            $initial = strtoupper(mb_substr($r['khach_hang'], 0, 1));
        ?>
        <tr onclick="selectRow(this)"
            ...
            <td><?=date('d/m/Y',strtotime($r['ngay_thu']))?></td>
            <td class="td-left">
                <div class="kh-cell">
                    <div class="kh-av"><?=$initial?></div>
                    <span><?=htmlspecialchars($r['khach_hang']??'')?></span>
                </div>
            </td>
            <td><span class="ct-badge"><?=htmlspecialchars($r['ma_chung_tu']??'')?></span></td>
            <td class="td-left"><?=htmlspecialchars($r['dien_giai']??'')?></td>
            <td class="td-left"><?=htmlspecialchars($r['nguoi_tra']??'')?></td>
            <td class="money-red"><?=number_format($r['so_tien_phai_thu'])?></td>
            <td class="money-green"><?=number_format($r['so_tien_da_thu'])?></td>
            <td><?= $con_lai > 0
                ? '<span class="status-debt">Còn '.number_format($con_lai).'</span>'
...
function tinhConLai(){
    ...
    el.style.color=cl>0?'#c62828':'#166534';
}
...
function filterTable(){
    ...
    document.querySelectorAll('#tableBody tr').forEach(row=>{
        ...
        if(fct&&!cells[9]?.innerText.includes('Còn'))show=false;
...
function resetFilter(){
    ...
    document.getElementById('f_chuathu').checked=false;
    filterTable();
}
