...
$result = $conn->query("SELECT * FROM danh_muc_lai_xe ORDER BY id ASC");
$rows = [];
...
<head>
    ...
    <title>Danh mục Lái xe – Hà Linh</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .driver-name { font-weight: 600; color: #1a1f36; }
        ...
        .salary { font-weight: 700; color: #1e40af; }

        /* Nghỉ badge */
        .badge-nghi   { background: #fee2e2; color: #c62828; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; }
        ...
        .modal-box { background: #fff; border-radius: 16px; width: 480px; max-width: 95vw; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,.2); animation: slideUp .22s ease; }
...
<div class="page-wrapper">
    ...
    <div class="card">
        <div class="card-header">
            <h1>🧑‍✈️ Danh mục Lái xe</h1>
            <p>Quản lý thông tin lái xe và lương trách nhiệm</p>
        </div>

        <div class="toolbar">
            <button class="btn btn-primary" onclick="openModal('form_lai_xe.php?action=add','THÊM MỚI LÁI XE')">
        ...
        <div class="search-wrap">
            <input type="text" class="search-input" id="searchInput" placeholder="Tìm theo tên lái xe..." oninput="filterRows()">
        </div>
        ...
        <div class="table-wrap">
            <table id="driverTable">
                ...
                <tbody id="tableBody">
                <?php if (empty($rows)): ?>
                <tr><td colspan="8"><div class="empty">Chưa có lái xe nào</div></td></tr>
                <?php else: $stt = 1; foreach ($rows as $row):
                    $initial = strtoupper(mb_substr($row['ten_lai_xe'], 0, 1));
                    $daNghi = !empty($row['da_nghi']) && $row['da_nghi'];
                ?>
                <tr onclick="selectRow(this, <?= $row['id'] ?>)">
                    <td><input type="radio" name="driver_id" value="<?= $row['id'] ?>" onclick="event.stopPropagation(); selectRow(this.closest('tr'), <?= $row['id'] ?>)"></td>
                    <td><?= $stt++ ?></td>
                    <td>
                    ...
                    <td><span class="salary"><?= number_format($row['luong_trach_nhiem']) ?></span></td>
                    <td>
                        <?php if ($daNghi): ?>
                    ...
                    <td><span class="date-text"><?= $row['ngay_nghi'] ? date('d/m/Y', strtotime($row['ngay_nghi'])) : '—' ?></span></td>
                </tr>
        ...
        <div class="table-footer">
            <span>Tổng: <strong id="rowCount"><?= count($rows) ?></strong> lái xe</span>
...
<div id="driverModal" class="modal">
    <div class="modal-box">
        <div class="modal-head">
            <h2 id="modalTitle">THÔNG TIN LÁI XE</h2>
            <button class="modal-close" onclick="closeModal()">×</button>
        </div>
        <iframe id="driverIframe" src="" style="width:100%;height:420px;border:none;"></iframe>
    </div>
</div>
...
function selectRow(row, id) {
    document.querySelectorAll('#driverTable tbody tr').forEach(r => r.classList.remove('selected'));
    row.classList.add('selected');
    selectedId = id;
    const name = row.querySelector('.driver-name')?.innerText || '';
...
function openModal(url, title) {
    ...
    document.getElementById('driverIframe').src = url;
...
function editSelected() {
    if (selectedId) openModal('form_lai_xe.php?action=edit&id=' + selectedId, 'CHỈNH SỬA LÁI XE');
    else alert('Vui lòng chọn một lái xe!');
}
function deleteSelected() {
    if (!selectedId) { alert('Vui lòng chọn một lái xe để xóa!'); return; }
    if (confirm('Bạn có chắc chắn muốn xóa lái xe này?'))
        window.location.href = 'xoa_lai_xe.php?id=' + selectedId;
}
...
document.addEventListener('keydown', function(e) {
    if (!e.altKey) return;
    if (e.key.toLowerCase() === 'a') { e.preventDefault(); openModal('form_lai_xe.php?action=add','THÊM MỚI LÁI XE'); }
